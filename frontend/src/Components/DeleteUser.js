...
const DeleteUser = ({setOpenUserDelete,openUserDelete,name,email,role,userId,callFunc}) => {
    const handleDelete= async()=>{
        const response=await fetch(summaryAPI.userDelete.url,{
            method:summaryAPI.userDelete.method,
            headers:{
  ...
  return (
    <div className=" fixed top-0 bottom-0 left-0 right-0 flex w-full h-full z-10  justify-between items-center bg-slate-200 bg-opacity-10">
      <div className="bg-white shadow-md p-4 w-full max-w-sm mx-auto">
       ...
       <h1 className="pb-4 text-lg font-medium">Delete User</h1>
        <p>Name: {name}</p>
        <p>Email: {email}</p>
          <p>Role: {role}</p>
       </div>
